    ...
    public class BeautifyEditor : VolumeComponentEditor {
        ...
        public override void OnInspectorGUI () {
            ...
            EditorGUILayout.BeginVertical();
            {
                ...
                if (GUILayout.Button(new GUIContent("Clear Effects", "Clears all effect overrides"), EditorStyles.miniButton)) {
                    if (EditorUtility.DisplayDialog("Clear Effects", "Do you want to clear all effects?", "Yes", "Cancel")) {
                        ...
                        beautify.smhShadows.overrideState = false;
                        beautify.smhMidtones.overrideState = false;
                        beautify.smhHighlights.overrideState = false;
                ...
                if (GUILayout.Button(new GUIContent("Quick Settings", "Applies a default set of effects including color improvement, vignette and bloom."), EditorStyles.miniButton)) {
                    if (EditorUtility.DisplayDialog("Quick Settings", "Do you want to apply a collection of example effect settings (including color improvements, dithering, vignette and bloom)?\nYou can adjust them later as you wish.", "Yes", "No")) {
            ...
            if (serializedObject.ApplyModifiedProperties()) {
                if (beautify.directWrite.value != prevDirectWrite || (beautify.bloomExclusionLayerMask.overrideState ? (int)beautify.bloomExclusionLayerMask.value : 0) != prevBloomExclusionLayerMask || beautify.outline.value) {
                    EditorApplication.delayCall += RepaintAllViews;
                }
            }
        ...
        void DrawPropertyField (SerializedDataParameter property, MemberInfo field, bool indent) {
            ...
            if (field.GetCustomAttribute<Beautify.ShowStrippedLabel>() != null) {
                ...
                if (isEnabled) {
                    ...
                    if ((fieldName == "ditherIntensity" && strippedKeywords.Contains(ShaderParams.SKW_DITHER)) ||
                        (fieldName == "tonemap" &&
                        ...
                        ((fieldName == "bloomIntensity" || fieldName == "sunFlaresIntensity") && strippedKeywords.Contains(ShaderParams.SKW_BLOOM)) ||
                        ...
                        (fieldName == "smhShadows" && strippedKeywords.Contains(ShaderParams.SKW_SMH)) ||
                        (fieldName == "colorTempBlend" && strippedKeywords.Contains(ShaderParams.SKW_COLOR_TWEAKS))) {
