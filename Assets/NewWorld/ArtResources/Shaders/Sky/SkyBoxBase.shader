Shader "NewWorld/Sky/SkyBoxBase"
{
    ...
    SubShader
    {
        ...
        {
            Name "NewWorldUnlit"
            // DrawSkybox is issued by Unity's skybox path, not NWRP DrawRenderers.
            // Use the built-in Always tag so the pass is accepted as a skybox pass.
            ...
            float3 SafeNormalizeDirection(float3 direction, float3 fallbackDirection)
            {
                float directionLengthSq = dot(direction, direction);
                float fallbackLengthSq = max(dot(fallbackDirection, fallbackDirection), 1e-4);
                return directionLengthSq > 1e-4
                    ? direction * rsqrt(directionLengthSq)
                    : fallbackDirection * rsqrt(fallbackLengthSq);
            }

            float3 GetSkySunDirection()
            {
                // Extension point: a future sky time-of-day feature can still upload _SunDir.
                // Until then, use NWRP's main directional light global so material controls
                // that depend on sun height/view angle are not locked at zero.
                float3 mainLightDirection = _MainLightPosition.xyz;
                float mainLightValid = step(1e-4, dot(mainLightDirection, mainLightDirection));
                mainLightDirection = lerp(float3(0.35, 0.75, 0.5), mainLightDirection, mainLightValid);
                return SafeNormalizeDirection(_SunDir, mainLightDirection);
            }

            float3 GetSkyMoonDirection(float3 sunDirection)
            {
                // Extension point: upload _MoonDir for authored moon phase/orbit control.
                return SafeNormalizeDirection(_MoonDir, -sunDirection);
            }

            float3 ApplyMoonSpace(float3 normal)
            {
                float matrixSignal =
                    dot(abs(_MoonSpaceMatrix[0].xyz), 1.0.xxx) +
                    dot(abs(_MoonSpaceMatrix[1].xyz), 1.0.xxx) +
                    dot(abs(_MoonSpaceMatrix[2].xyz), 1.0.xxx);
                float hasMoonSpace = step(1e-4, matrixSignal);
                float3 transformed = mul(_MoonSpaceMatrix, float4(normal, 0)).xyz;
                return lerp(normal, transformed, hasMoonSpace);
            }
            ...
            float3 GetMoonTexture(float3 normal)
            {
                ...
                float3 uvw = ApplyMoonSpace(normal);
            ...
            float4 Fragment (v2f IN) : SV_TARGET
            {
                ...
                float3 sunDir = GetSkySunDirection();
                float3 moonDir = GetSkyMoonDirection(sunDir);

                // Main angles
                float sunViewDot = dot(sunDir, viewDir);
                float sunZenithDot = sunDir.y;
                ...
                float sunHeight01 = saturate(sunDir.y  * 4); //对应太阳高度控制白天阶段
                ...
                float3 finalSkyColor = lerp(skyColor, cloudColor * (1 - cloudMask * _CloudIntensity), sunHeight01); //cloudOverlay + cloudColor
                skyColor = finalSkyColor;

                // The sun
                float sunMask = GetSunMask(sunViewDot, _SunRadius);
                float3 sunColor = _MainLightColor.rgb * sunMask;
                float3 sunHalo = GetSunHalo(viewDir, sunDir, _MainLightColor.rgb, _SunFalloff) * _SunIntensity; 

                // The moon
                float moonIntersect = sphIntersect(viewDir, moonDir, _MoonRadius);
                float moonMask = moonIntersect > -1 ? 1 : 0;
                float3 moonNormal = normalize(viewDir * moonIntersect - moonDir);
                float moonNdotL = saturate(dot(moonNormal, sunDir));
                ...
                float3 starColor = _StarCubeMap.SampleBias(sampler_StarCubeMap, starUVW, -1.0).rgb;
