Shader "Effects/Alpha2Mask"
{
    Properties
    {
        ...
        _MaskTex2 ("Mask Tex 2", 2D) = "white" {}
        _MaskTiling_Offset2 ("Mask Tiling & Offset 2", Vector) = (1,1,0,0)
        [Toggle] _MaskLoop2 ("Mask Loop 2", Float) = 0
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            half4 Frag(Varyings input) : SV_Target
            {
                ...
                float2 mask2UV = ApplyTilingOffset(input.uv, _MaskTiling_Offset2, _MaskLoop2);
                float2 mask2BlendUV = ApplyTilingOffset(input.blendUV.xy, _MaskTiling_Offset2, _MaskLoop2);

                half4 mainTex = SampleParticleTexture(TEXTURE2D_ARGS(_MainTex, sampler_MainTex), mainUV, mainBlendUV, blend);
                half4 maskTex = SampleParticleTexture(TEXTURE2D_ARGS(_MaskTex, sampler_MaskTex), maskUV, maskBlendUV, blend);
                half4 maskTex2 = SampleParticleTexture(TEXTURE2D_ARGS(_MaskTex2, sampler_MaskTex2), mask2UV, mask2BlendUV, blend);

                half4 baseColor = mainTex * input.color * _Color;
                half maskA = pow(saturate(maskTex.r), _Power1) * _Power2;
                half maskB = pow(saturate(maskTex2.r), _Power1) * _Power2;
                return half4(baseColor.rgb, baseColor.a * maskA * maskB);
            }
            ENDHLSL
        }
    }
